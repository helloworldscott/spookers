...
export class PlayerController {
  ...
  update(dt) {
    ...
    const forwardInput = (this.move.f - this.move.b);
    const strafeInput = (this.move.r - this.move.l);
    ...
    this.direction.copy(forward).multiplyScalar(forwardInput).addScaledVector(right, strafeInput);
    if (this.direction.lengthSq() > 0) this.direction.normalize();

    this.camera.position.x = THREE.MathUtils.clamp(this.camera.position.x + this.direction.x * speed * dt, -this.bounds, this.bounds);
    this.camera.position.z = THREE.MathUtils.clamp(this.camera.position.z + this.direction.z * speed * dt, -this.bounds, this.bounds);
